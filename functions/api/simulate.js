/**
 * Cloudflare Pages Function - 模拟计算 API
 * 路径: /api/simulate
 */
...
function generateSimulatedNutrients(lon, lat) {
  ...
  return {
    AN: { value: Math.round(AN * 10) / 10, nutrient_level: getNutrientLevel(AN, 'AN') },
    AP: { value: Math.round(AP * 10) / 10, nutrient_level: getNutrientLevel(AP, 'AP') },
    AK: { value: Math.round(AK * 10) / 10, nutrient_level: getNutrientLevel(AK, 'AK') }
  };
}
